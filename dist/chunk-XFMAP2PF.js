...
import { relative as relative2, dirname as dirname2, basename as basename2, extname as extname2, resolve as resolve$1, join } from "path";
import { posix, isAbsolute as isAbsolute2, resolve as resolve2, win32 } from "path";
...
var isInteger = (property3) => typeof property3 === "string" && /^\d+$/.test(property3);
...
var MetaProperty = class extends NodeBase {
  ...
  render(code, renderOptions) {
    ...
    const { scope: { context: { module } }, meta: { name }, metaProperty: metaProperty3, parent, preliminaryChunkId, referenceId, start, end } = this;
    ...
    if (referenceId) {
      ...
      const relativePath = normalize(relative2(dirname2(chunkId), fileName));
...
var getGenericImportMetaMechanism = (getUrl) => (property3, { chunkId }) => {
...
var PanicError2 = class extends NodeBase {
  initialise() {
...
var ParseError2 = class extends NodeBase {
  initialise() {
...
`);
  ...
  getPreserveModulesChunkNameFromModule(module) {
    ...
    if (isAbsolute(idWithoutExtension)) {
      ...
      return this.outputOptions.virtualDirname.replace(/\/$/, "") + "/" + basename2(idWithoutExtension);
    }
  }
...
var textEncoder;
var getHash64 = (input) => (0, import_native2.xxhashBase64Url)(ensureBuffer(input));
var getHash36 = (input) => (0, import_native2.xxhashBase36)(ensureBuffer(input));
var getHash16 = (input) => (0, import_native2.xxhashBase16)(ensureBuffer(input));
var hasherByType = {
  base36: getHash36,
  base64: getHash64,
  hex: getHash16
...
function emitSourceMapAndGetComment(fileName, map, pluginDriver, { sourcemap, sourcemapBaseUrl }) {
  ...
  if (sourcemap === "inline") {
    ...
    const sourcemapFileName = basename2(fileName);
...
async function resolveId(source, importer, preserveSymlinks, pluginDriver, moduleLoaderResolveId, skip, customOptions, isEntry, attributes, importerAttributes, fs2) {
  ...
  if (importer !== void 0 && !isAbsolute(source) && source[0] !== ".")
    return null;
  return addJsExtensionIfNecessary(importer ? resolve$1(dirname2(importer), source) : resolve$1(source), preserveSymlinks, fs2);
}
...
async function findFile(file, preserveSymlinks, fs2) {
  try {
    ...
    if (preserveSymlinks && stats.isSymbolicLink() || stats.isFile()) {
      ...
      const files = await fs2.readdir(dirname2(file));
...
var fs = /* @__PURE__ */ _mergeNamespaces({
  __proto__: null
}, [promises]);
...
async function writeOutputFile(outputFile, outputOptions, { fs: { mkdir, writeFile } }) {
  const fileName = resolve$1(outputOptions.dir || dirname2(outputOptions.file), outputFile.fileName);
  await mkdir(dirname2(fileName), { recursive: true });
...
var { bold, cyan, dim, red } = pc.createColors(env.FORCE_COLOR !== "0" && !env.NO_COLOR);
var fsEvents;
...
rollup/dist/es/shared/node-entry.js:
  ...
  *)
*/
//# sourceMappingURL=chunk-XFMAP2PF.js.map