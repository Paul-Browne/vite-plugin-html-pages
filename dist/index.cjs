...
var require_native = __commonJS({
  "node_modules/rollup/dist/native.js"(exports2, module2) {
    ...
    var { platform, arch, report } = require("process");
    ...
    var { parse: parse2, parseAsync: parseAsync3, xxhashBase64Url: xxhashBase64Url2, xxhashBase36: xxhashBase362, xxhashBase16: xxhashBase162 } = requireWithFriendlyError(
      existsSync(path5.join(__dirname, localName)) ? localName : `@rollup/rollup-${packageBase}`
    );
...
If this is important to you, please consider supporting Rollup to make a native build for your platform and architecture available.`
    ...
    module2.exports.xxhashBase16 = xxhashBase162;
  }
});
...
function getAliasName(id) {
  const base2 = (0, import_node_path3.basename)(id);
  return base2.slice(0, Math.max(0, base2.length - (0, import_node_path3.extname)(id).length));
}
function relativeId(id) {
  if (!isAbsolute(id))
    return id;
  return relative((0, import_node_path3.resolve)(), id);
}
...
function getImportPath(importerId, targetPath, stripJsExtension, ensureFileName) {
  ...
  let relativePath = normalize(relative((0, import_node_path3.dirname)(importerId), targetPath));
  if (stripJsExtension && relativePath.endsWith(".js")) {
    relativePath = relativePath.slice(0, -3);
  }
  if (ensureFileName) {
    if (relativePath === "")
      return "../" + (0, import_node_path3.basename)(targetPath);
    if (UPPER_DIR_REGEX.test(relativePath)) {
      return [...relativePath.split("/"), "..", (0, import_node_path3.basename)(targetPath)].join("/");
    }
  }
...
function logMissingExport(binding, importingModule, exporter, missingButExportExists) {
  ...
  const isJson = (0, import_node_path3.extname)(exporter) === ".json";
  return {
    ...baseLog,
...
function logImplicitDependantIsNotIncluded(module2) {
...
function logModuleLevelDirective(directive3, id) {
  return {
    code: MODULE_LEVEL_DIRECTIVE,
    id,
    message: `Module level directives cause errors when bundled, "${directive3}" in "${relativeId(id)}" was ignored.`
  };
}
...
var init_parseAst = __esm({
  "node_modules/rollup/dist/es/shared/parseAst.js"() {
    ...
    URL_LOAD = "plugin-development/#load";
    URL_TRANSFORM = "plugin-development/#transform";
    symbolAugmented = /* @__PURE__ */ Symbol("augmented");
...
function getGlobalAtPath(path5) {
  ...
  for (const pathSegment of path5) {
    ...
    if (!currentGlobal) {
      return path5[0] === "Symbol" && path5.length === 2 ? UNKNOWN_WELL_KNOWN : null;
    }
  }
...
function getChainElementLiteralValueAtPath(element, object, path5, recursionTracker, origin) {
  ...
  return element.getLiteralValueAtPath(path5, recursionTracker, origin);
}
...
function resolveNamespaceVariables(baseVariable, path5, astContext) {
  ...
  if (!variable) {
    if (path5.length === 1) {
      ...
      astContext.log(LOGLEVEL_WARN, logMissingExport(exportName, astContext.module.id, fileName, !!options?.missingButExportExists), path5[0].pos);
      return "undefined";
    }
    return null;
  }
...
function requireUtils() {
  ...
  (function(exports$1) {
    ...
    exports$1.basename = (path5, { windows } = {}) => {
...
function convertProgram2(buffer, parent, parentScope) {
...
function convertNodeList2(parent, parentScope, position, buffer) {
  if (position === 0)
    return EMPTY_ARRAY;
  const length = buffer[position++];
  const list = new Array(length);
...
function getAttributesFromImportExpression(node) {
  ...
  const assertFields = assertProperty.properties.map((property3) => {
    ...
    context.module.log(LOGLEVEL_WARN, logImportAttributeIsInvalid(context.module.id), property3.start);
    return null;
...
function deconflictChunk(modules, dependenciesToBeDeconflicted, imports, usedNames, format, interop, preserveModules, externalLiveBindings, chunkByModule, externalChunkByModule, syntheticExports, exportNamesByVariable, accessedGlobalsByScope, includedNamespaces) {
  ...
  for (const module2 of reversedModules) {
    module2.scope.addUsedOutsideNames(usedNames, accessedGlobalsByScope);
  }
...
function deconflictTopLevelVariables(usedNames, modules, includedNamespaces) {
  var _a;
  for (const module2 of modules) {
    (_a = module2.info).safeVariableNames || (_a.safeVariableNames = {});
    for (const variable of module2.scope.variables.values()) {
      ...
      !(variable.renderBaseName || variable instanceof ExportDefaultVariable && variable.getOriginalVariable() !== variable)) {
        const cachedSafeVariableName = Object.getOwnPropertyDescriptor(module2.info.safeVariableNames, variable.name)?.value;
        ...
        variable.setRenderNames(null, getSafeName(variable.name, usedNames, variable.forbiddenNames));
        module2.info.safeVariableNames[variable.name] = variable.renderName;
      }
    }
...
function makeUnique(name, { [lowercaseBundleKeys]: reservedLowercaseBundleKeys }) {
  if (!reservedLowercaseBundleKeys.has(name.toLowerCase()))
    return name;
  const extension = (0, import_node_path4.extname)(name);
...
function getPredefinedChunkNameFromModule(module2) {
  return module2.chunkNames.find(({ isUserDefined }) => isUserDefined)?.name ?? module2.chunkNames[0]?.name;
}
...
function* getModulesWithDependentEntriesAndHandleTLACycles(dependentEntriesByModule, modulesInManualChunks, chunkDefinitions) {
  for (const [module2, dependentEntries] of dependentEntriesByModule) {
    if (!modulesInManualChunks.has(module2)) {
      if (module2.cycles.size > 0 && module2.includedTopLevelAwaitingDynamicImporters.size > 0) {
...
function getChunksWithSameDependentEntriesAndCorrelatedAtoms(chunkAtoms, staticDependencyAtomsByEntry, alreadyLoadedAtomsByEntry, minChunkSize) {
  ...
  for (const { dependentEntries, modules } of chunkAtoms) {
    ...
    for (const module2 of modules) {
      ...
      if (module2.isIncluded()) {
        pure && (pure = !module2.hasEffects());
        atomSize += minChunkSize > 1 ? module2.estimateSize() : 1;
      }
    }
    if (!pure) {
...
function commondir(files) {
  if (files.length === 0)
    return "/";
  if (files.length === 1)
    return (0, import_node_path4.dirname)(files[0]);
...
function analyseModuleExecution(entryModules) {
  ...
  const analyseModule = (module2) => {
    if (module2 instanceof Module) {
      ...
      for (const { node: { resolution, scope } } of module2.dynamicImports) {
...
function getCollapsedSourcemap(id, originalCode, originalSourcemap, sourcemapChain, linkMap) {
  ...
  if (originalSourcemap) {
    ...
    const directory = (0, import_node_path4.dirname)(id) || ".";
    const sourceRoot = originalSourcemap.sourceRoot || ".";
    const baseSources = sources.map((source2, index) => new Source((0, import_node_path4.resolve)(directory, sourceRoot, source2), sourcesContent[index]));
...
function collapseSourcemaps(file, map, modules, bundleSourcemapChain, excludeContent, log) {
  ...
  if (file) {
    const directory = (0, import_node_path4.dirname)(file);
    sources = sources.map((source2) => (0, import_node_path4.relative)(directory, source2));
    file = (0, import_node_path4.basename)(file);
  }
  for (const module2 of modules) {
    resetSourcemapCache(module2.originalSourcemap, module2.sourcemapChain);
  }
...
async function transformChunk(magicString, fileName, usedModules, chunkGraph, options, outputPluginDriver, log) {
  ...
  if (sourcemap) {
    ...
    let resultingFile;
    if (file)
      resultingFile = (0, import_node_path4.resolve)(sourcemapFile || file);
    else if (dir)
      resultingFile = (0, import_node_path4.resolve)(dir, fileName);
    else
...
function emitSourceMapAndGetComment(fileName, map, pluginDriver, { sourcemap, sourcemapBaseUrl }) {
  ...
  if (sourcemap === "inline") {
    ...
    const sourcemapFileName = (0, import_node_path4.basename)(fileName);
...
function addModuleToManualChunk(alias, module2, manualChunkAliasByEntry) {
  ...
  if (typeof existingAlias === "string" && existingAlias !== alias) {
    return error(logCannotAssignModuleToChunk(module2.id, alias, existingAlias));
  }
  manualChunkAliasByEntry.set(module2, alias);
}
function isFileNameOutsideOutputDirectory(fileName) {
  const normalized = (0, import_node_path4.join)(fileName).replaceAll("\\", "/");
...
async function resolveId(source, importer, preserveSymlinks, pluginDriver, moduleLoaderResolveId, skip, customOptions, isEntry, attributes, importerAttributes, fs3) {
  ...
  if (importer !== void 0 && !isAbsolute(source) && source[0] !== ".")
    return null;
  return addJsExtensionIfNecessary(importer ? (0, import_node_path4.resolve)((0, import_node_path4.dirname)(importer), source) : (0, import_node_path4.resolve)(source), preserveSymlinks, fs3);
}
async function addJsExtensionIfNecessary(file, preserveSymlinks, fs3) {
  return await findFile(file, preserveSymlinks, fs3) ?? await findFile(file + ".mjs", preserveSymlinks, fs3) ?? await findFile(file + ".js", preserveSymlinks, fs3);
}
async function findFile(file, preserveSymlinks, fs3) {
  try {
    ...
    if (preserveSymlinks && stats.isSymbolicLink() || stats.isFile()) {
      const name = (0, import_node_path4.basename)(file);
      const files = await fs3.readdir((0, import_node_path4.dirname)(file));
...
async function transform(source, module2, pluginDriver, options) {
  const id = module2.id;
  const sourcemapChain = [];
  ...
  try {
    code = await pluginDriver.hookReduceArg0("transform", [
      ...
      id,
      {
        attributes: module2.info.attributes
      }
...
function normalizeRelativeExternalId(source, importer) {
  return isRelative(source) ? importer ? (0, import_node_path4.resolve)(importer, "..", source) : (0, import_node_path4.resolve)(source) : source;
}
function addChunkNamesToModule(module2, { fileName, name }, isUserDefined, priority) {
  if (fileName !== null) {
    ...
    let namePosition = 0;
    while (module2.chunkNames[namePosition]?.priority < priority)
      namePosition++;
...
function generateAssetFileName(name, names, source, originalFileName, originalFileNames, sourceHash, outputOptions, bundle, inputOptions) {
  ...
  return makeUnique(renderNamePattern(typeof outputOptions.assetFileNames === "function" ? outputOptions.assetFileNames({
    ...
    ext: () => (0, import_node_path4.extname)(emittedName).slice(1),
    extname: () => (0, import_node_path4.extname)(emittedName),
...
function getMatcherString(glob, cwd) {
  ...
  const resolved = (0, import_node_path4.resolve)(cwd, glob);
...
async function writeOutputFile(outputFile, outputOptions, { fs: { mkdir, writeFile } }) {
  const fileName = (0, import_node_path4.resolve)(outputOptions.dir || (0, import_node_path4.dirname)(outputOptions.file), outputFile.fileName);
  await mkdir((0, import_node_path4.dirname)(fileName), { recursive: true });
...
var init_node_entry = __esm({
  "node_modules/rollup/dist/es/shared/node-entry.js"() {
    ...
    NON_WHITESPACE = /\S/g;
    WHITESPACE = /\s/;
    UnknownKey = /* @__PURE__ */ Symbol("Unknown Key");
    UnknownNonAccessorKey = /* @__PURE__ */ Symbol("Unknown Non-Accessor Key");
    UnknownInteger = /* @__PURE__ */ Symbol("Unknown Integer");
    UnknownWellKnown = /* @__PURE__ */ Symbol("Unknown Well-Known");
    SymbolToStringTag = /* @__PURE__ */ Symbol("Symbol.toStringTag");
    SymbolDispose = /* @__PURE__ */ Symbol("Symbol.asyncDispose");
    SymbolAsyncDispose = /* @__PURE__ */ Symbol("Symbol.dispose");
    SymbolHasInstance = /* @__PURE__ */ Symbol("Symbol.hasInstance");
    WELL_KNOWN_SYMBOLS_LIST = [
      SymbolToStringTag,
      SymbolDispose,
      SymbolAsyncDispose,
      SymbolHasInstance
    ];
    WELL_KNOWN_SYMBOLS = new Set(WELL_KNOWN_SYMBOLS_LIST);
    isAnyWellKnown = (v) => WELL_KNOWN_SYMBOLS.has(v) || v === UnknownWellKnown;
    TREE_SHAKEABLE_SYMBOLS_LIST = [SymbolHasInstance, SymbolDispose, SymbolAsyncDispose];
    TREE_SHAKEABLE_SYMBOLS = new Set(TREE_SHAKEABLE_SYMBOLS_LIST);
    isConcreteKey = (v) => typeof v === "string" || WELL_KNOWN_SYMBOLS.has(v);
    EMPTY_PATH = [];
    UNKNOWN_PATH = [UnknownKey];
    UNKNOWN_NON_ACCESSOR_PATH = [UnknownNonAccessorKey];
    UNKNOWN_INTEGER_PATH = [UnknownInteger];
    INSTANCEOF_PATH = [SymbolHasInstance];
    EntitiesKey = /* @__PURE__ */ Symbol("Entities");
    ...
    IncludedFullPathTracker = class {
      ...
      includePathAndGetIfIncluded(path5) {
        let included = true;
        let parent = this;
    ...
    IncludedTopLevelPathTracker = class {
      ...
      includePathAndGetIfIncluded(path5) {
        ...
        const [firstPathSegment, secondPathSegment] = path5;
        if (!firstPathSegment) {
          return included;
        }
    ...
    UnknownValue = /* @__PURE__ */ Symbol("Unknown Value");
    UnknownTruthyValue = /* @__PURE__ */ Symbol("Unknown Truthy Value");
    UnknownFalsyValue = /* @__PURE__ */ Symbol("Unknown Falsy Value");
    ...
    PureFunctionKey = /* @__PURE__ */ Symbol("PureFunction");
    ...
    Variable = class extends ExpressionEntity {
       ...
       */
      includePath(path5, context) {
        this.included = true;
        this.renderedLikeHoisted?.includePath(path5, context);
      }
      /**
    ...
    ExternalVariable = class extends Variable {
      ...
      includePath(path5, context) {
        super.includePath(path5, context);
    ...
    ExternalModule = class {
      ...
      getVariableForExportName(name, { importChain }) {
        ...
        for (const module2 of importChain) {
          getOrCreate(this.importersByExportedName, name, getNewSet).add(module2);
        }
        if (declaration)
          return [declaration];
    ...
    IS_SKIPPED_CHAIN = /* @__PURE__ */ Symbol("IS_SKIPPED_CHAIN");
    ...
    UNDEFINED_EXPRESSION = new class UndefinedExpression extends ExpressionEntity {
      getLiteralValueAtPath(path5) {
        return path5.length > 0 ? UnknownValue : void 0;
      }
    }();
    returnsUnknown = {
      value: {
    ...
    Method = class extends ExpressionEntity {
      ...
      deoptimizeArgumentsOnInteractionAtPath({ args, type }, path5) {
        if (type === INTERACTION_CALLED && path5.length === 0) {
      ...
      hasEffectsOnInteractionAtPath(path5, { args, type }, context) {
    ...
    ObjectEntity = class extends ExpressionEntity {
      ...
      deoptimizeArgumentsOnInteractionAtPath(interaction, path5, recursionTracker) {
        ...
        (type === INTERACTION_CALLED || path5.length > 1) && (this.hasUnknownDeoptimizedProperty || isConcreteKey(key) && this.deoptimizedPaths.get(key))) {
          deoptimizeInteraction(interaction);
          return;
        }
      ...
      deoptimizePath(path5) {
        ...
        this.prototypeExpression?.deoptimizePath(path5.length === 1 ? [path5[0], UnknownKey] : path5);
      }
      ...
      include(context, includeChildrenRecursively) {
        ...
        for (const property3 of this.allProperties) {
          if (includeChildrenRecursively || property3.shouldBeIncluded(context) || this.alwaysIncludedProperties.has(property3)) {
            property3.include(context, includeChildrenRecursively);
          }
        }
      ...
      includePath(path5, context) {
        ...
        this.prototypeExpression?.includePath(path5, context);
      }
      buildPropertyMaps(properties) {
        ...
        for (let index = properties.length - 1; index >= 0; index--) {
          ...
          if (isAnyWellKnown(key) && !TREE_SHAKEABLE_SYMBOLS.has(key)) {
            alwaysIncludedProperties.add(property3);
          ...
          if (isConcreteKey(key)) {
            if (kind === "set") {
              if (!propertiesAndSettersByKey.has(key)) {
                propertiesAndSettersByKey.set(key, [property3, ...unmatchablePropertiesAndSetters]);
                settersByKey.set(key, [property3, ...unmatchableSetters]);
              }
            } else if (kind === "get") {
              if (!propertiesAndGettersByKey.has(key)) {
                propertiesAndGettersByKey.set(key, [property3, ...unmatchablePropertiesAndGetters]);
                gettersByKey.set(key, [property3, ...unmatchableGetters]);
              }
            } else {
              if (!propertiesAndSettersByKey.has(key)) {
                propertiesAndSettersByKey.set(key, [property3, ...unmatchablePropertiesAndSetters]);
    ...
    ARRAY_PROTOTYPE = new ObjectEntity(/* @__PURE__ */ new Map([
    ...
    ValueProperties = /* @__PURE__ */ Symbol("Value Properties");
    getUnknownValue = () => UnknownValue;
    returnFalse = () => false;
    returnTrue = () => true;
    getWellKnownSymbol = (symbol) => ({
      __proto__: null,
      [ValueProperties]: {
    ...
    limitConcatenatedPathDepth = (path1, path22) => {
      ...
      const { length: length2 } = path22;
      return length1 === 0 ? path22 : length2 === 0 ? path1 : length1 + length2 > MAX_PATH_DEPTH ? [...path1, ...path22.slice(0, MAX_PATH_DEPTH - 1 - path1.length), "UnknownKey"] : [...path1, ...path22];
    };
    LocalVariable = class extends Variable {
      ...
      addDeclaration(identifier3, init) {
        this.declarations.push(identifier3);
      ...
      deoptimizeArgumentsOnInteractionAtPath(interaction, path5, recursionTracker) {
        if (this.isReassigned || path5.length + this.initPath.length > MAX_PATH_DEPTH) {
          deoptimizeInteraction(interaction);
          return;
        }
        recursionTracker.withTrackedEntityAtPath(path5, this.init, () => {
          this.init.deoptimizeArgumentsOnInteractionAtPath(interaction, [...this.initPath, ...path5], recursionTracker);
        }, void 0);
      }
      deoptimizePath(path5) {
        ...
        if (path5.length === 0) {
          ...
          this.init.deoptimizePath(limitConcatenatedPathDepth(this.initPath, path5));
        }
      }
      getLiteralValueAtPath(path5, recursionTracker, origin) {
        if (this.isReassigned || path5.length + this.initPath.length > MAX_PATH_DEPTH) {
        ...
        return recursionTracker.withTrackedEntityAtPath(path5, this.init, () => {
          ...
          return this.init.getLiteralValueAtPath([...this.initPath, ...path5], recursionTracker, origin);
        }, UnknownValue);
      }
      getReturnExpressionWhenCalledAtPath(path5, interaction, recursionTracker, origin) {
        if (this.isReassigned || path5.length + this.initPath.length > MAX_PATH_DEPTH) {
        ...
        return recursionTracker.withTrackedEntityAtPath(path5, this.init, () => {
          this.expressionsToBeDeoptimized.push(origin);
          return this.init.getReturnExpressionWhenCalledAtPath([...this.initPath, ...path5], interaction, recursionTracker, origin);
        }, UNKNOWN_RETURN_EXPRESSION);
      }
      hasEffectsOnInteractionAtPath(path5, interaction, context) {
        if (path5.length + this.initPath.length > MAX_PATH_DEPTH) {
          return true;
        }
        switch (interaction.type) {
          case INTERACTION_ACCESSED: {
            if (this.isReassigned)
              return true;
            return !context.accessed.trackEntityAtPathAndGetIfTracked(path5, this) && this.init.hasEffectsOnInteractionAtPath([...this.initPath, ...path5], interaction, context);
          }
          case INTERACTION_ASSIGNED: {
            if (this.included)
              return true;
            ...
            if (this.isReassigned)
              return true;
            return !context.assigned.trackEntityAtPathAndGetIfTracked(path5, this) && this.init.hasEffectsOnInteractionAtPath([...this.initPath, ...path5], interaction, context);
          }
          case INTERACTION_CALLED: {
            if (this.isReassigned)
              return true;
            return !(interaction.withNew ? context.instantiated : context.called).trackEntityAtPathAndGetIfTracked(path5, interaction.args, this) && this.init.hasEffectsOnInteractionAtPath([...this.initPath, ...path5], interaction, context);
          }
        }
      }
      includePath(path5, context) {
        if (!this.includedPathTracker.includePathAndGetIfIncluded(path5)) {
          ...
          if (path5.length > 0) {
            this.init.includePath(limitConcatenatedPathDepth(this.initPath, path5), context);
      ...
      includePath(path5, context) {
        if (!this.included) {
          ...
          if (!this.deoptimized)
            this.applyDeoptimizations();
          if (this.variable !== null) {
            this.scope.context.includeVariableInModule(this.variable, path5, context);
          }
        } else if (path5.length > 0) {
          this.variable?.includePath(path5, context);
        }
      }
      includeCallArguments(interaction, context) {
    ...
    ObjectMember = class extends ExpressionEntity {
      constructor(object, path5) {
        super();
        this.object = object;
        this.path = path5;
      }
      deoptimizeArgumentsOnInteractionAtPath(interaction, path5, recursionTracker) {
        this.object.deoptimizeArgumentsOnInteractionAtPath(interaction, [...this.path, ...path5], recursionTracker);
      }
      deoptimizePath(path5) {
        this.object.deoptimizePath([...this.path, ...path5]);
      }
      getLiteralValueAtPath(path5, recursionTracker, origin) {
        return this.object.getLiteralValueAtPath([...this.path, ...path5], recursionTracker, origin);
      }
      getReturnExpressionWhenCalledAtPath(path5, interaction, recursionTracker, origin) {
        return this.object.getReturnExpressionWhenCalledAtPath([...this.path, ...path5], interaction, recursionTracker, origin);
      }
      hasEffectsOnInteractionAtPath(path5, interaction, context) {
        return this.object.hasEffectsOnInteractionAtPath([...this.path, ...path5], interaction, context);
      }
    };
    ...
    Scope = class {
       ...
       */
      addDeclaration(identifier3, context, init, destructuredInitPath, kind) {
        const name = identifier3.name;
        const existingVariable = this.hoistedVariables?.get(name) || this.variables.get(name);
        if (existingVariable) {
          if (kind === "var" && existingVariable.kind === "var") {
            existingVariable.addDeclaration(identifier3, init);
            return existingVariable;
          }
          context.error(logRedeclarationError(name), identifier3.start);
        }
        const newVariable = new LocalVariable(identifier3.name, identifier3, init, destructuredInitPath, context, kind);
    ...
    ChildScope = class _ChildScope extends Scope {
      ...
      addDeclaration(identifier3, context, init, destructuredInitPath, kind) {
        if (kind === "var") {
          ...
          if (existingVariable) {
            ...
            return context.error(logRedeclarationError(name), identifier3.start);
          }
          const declaredVariable = this.parent.addDeclaration(identifier3, context, init, destructuredInitPath, kind);
          declaredVariable.markInitializersForDeoptimization();
          this.addHoistedVariable(name, declaredVariable);
          return declaredVariable;
        }
        return super.addDeclaration(identifier3, context, init, destructuredInitPath, kind);
      }
    };
    ...
    ClassNode = class extends NodeBase {
      ...
      include(context, includeChildrenRecursively) {
        ...
        for (const decorator3 of this.decorators)
          decorator3.include(context, includeChildrenRecursively);
        if (this.id) {
          this.id.markDeclarationReached();
    ...
    ParameterVariable = class extends LocalVariable {
      ...
      addArgumentForDeoptimization(entity) {
        ...
        if (entity === UNKNOWN_EXPRESSION) {
          ...
          for (const { interaction, path: path5 } of this.deoptimizationInteractions) {
            entity.deoptimizeArgumentsOnInteractionAtPath(interaction, [...this.initPath, ...path5], SHARED_RECURSION_TRACKER);
          }
        }
      }
      ...
      getLiteralValueAtPath(path5, recursionTracker, origin) {
        ...
        return recursionTracker.withTrackedEntityAtPath(path5, knownValue, () => knownValue.getLiteralValueAtPath([...this.initPath, ...path5], recursionTracker, origin), UnknownValue);
      }
      hasEffectsOnInteractionAtPath(path5, interaction, context) {
        const { type } = interaction;
        if (this.isReassigned || type === INTERACTION_ASSIGNED || path5.length + this.initPath.length > MAX_PATH_DEPTH) {
          return super.hasEffectsOnInteractionAtPath(path5, interaction, context);
        }
        return !(type === INTERACTION_CALLED ? (interaction.withNew ? context.instantiated : context.called).trackEntityAtPathAndGetIfTracked(path5, interaction.args, this) : context.accessed.trackEntityAtPathAndGetIfTracked(path5, this)) && this.getKnownValue().hasEffectsOnInteractionAtPath([...this.initPath, ...path5], interaction, context);
      }
      deoptimizeArgumentsOnInteractionAtPath(interaction, path5) {
        if (path5.length >= 2 || this.argumentsToBeDeoptimized.has(UNKNOWN_EXPRESSION) || this.deoptimizationInteractions.length >= MAX_TRACKED_INTERACTIONS || path5.length === 1 && (this.deoptimizedFields.has(UnknownKey) || interaction.type === INTERACTION_CALLED && this.deoptimizedFields.has(path5[0])) || this.initPath.length + path5.length > MAX_PATH_DEPTH) {
          deoptimizeInteraction(interaction);
          return;
        }
        ...
        if (kind === "var") {
          ...
          if (existingVariable) {
            ...
            existingVariable.declarations[0].parent.type === CatchClause) {
              const declaredVariable2 = this.parent.parent.addDeclaration(identifier3, context, UNDEFINED_EXPRESSION, destructuredInitPath, kind);
          ...
          const declaredVariable = this.parent.parent.addDeclaration(identifier3, context, init, destructuredInitPath, kind);
          declaredVariable.markInitializersForDeoptimization();
          this.addHoistedVariable(name, declaredVariable);
          return declaredVariable;
        }
    ...
    ParameterScope = class extends ChildScope {
       ...
       */
      addParameterDeclaration(identifier3, argumentPath) {
        const { name, start } = identifier3;
        const existingParameter = this.variables.get(name);
        if (existingParameter) {
          return this.context.error(logDuplicateArgumentNameError(name), start);
        }
        const variable = new ParameterVariable(name, identifier3, argumentPath, this.context);
    ...
    FunctionBase = class extends NodeBase {
      ...
      deoptimizeArgumentsOnInteractionAtPath(interaction, path5, recursionTracker) {
        if (interaction.type === INTERACTION_CALLED && path5.length === 0) {
    ...
    FunctionNode = class extends FunctionBase {
      ...
      hasEffectsOnInteractionAtPath(path5, interaction, context) {
        if (this.annotationNoSideEffects && path5.length === 0 && interaction.type === INTERACTION_CALLED) {
          return false;
        }
    ...
    ExportDefaultDeclaration2 = class extends NodeBase {
      ...
      includePath(path5, context) {
        this.included = true;
        this.declaration.includePath(path5, context);
      }
      initialise() {
        super.initialise();
    ...
    getWithToStringTag = (symbols, fragment, { _, getObject }) => symbols ? `Object.defineProperty(${fragment},${_}Symbol.toStringTag,${_}${getToStringTagValue(getObject)})` : fragment;
    ...
    MemberExpression2 = class extends NodeBase {
      ...
      bind() {
        ...
        if (baseVariable?.isNamespace) {
          const resolvedVariable = resolveNamespaceVariables(baseVariable, path5.slice(1), this.scope.context);
          if (!resolvedVariable) {
            super.bind();
      ...
      deoptimizeArgumentsOnInteractionAtPath(interaction, path5, recursionTracker) {
        if (this.promiseHandler) {
          this.promiseHandler.deoptimizeArgumentsOnInteractionAtPath(interaction, path5, recursionTracker);
        } else if (this.variable) {
          this.variable.deoptimizeArgumentsOnInteractionAtPath(interaction, path5, recursionTracker);
        } else if (!this.isUndefined) {
          if (path5.length < MAX_PATH_DEPTH) {
            this.object.deoptimizeArgumentsOnInteractionAtPath(interaction, this.propertyKey === UnknownKey ? UNKNOWN_PATH : [this.propertyKey, ...path5], recursionTracker);
          } else {
            deoptimizeInteraction(interaction);
          }
        }
      ...
      deoptimizePath(path5) {
        ...
        if (this.variable) {
          ...
          this.object.deoptimizePath([
            ...
            ...path5.length < MAX_PATH_DEPTH ? path5 : [...path5.slice(0, MAX_PATH_DEPTH), UnknownKey]
          ]);
        }
      }
      getLiteralValueAtPath(path5, recursionTracker, origin) {
        ...
        if (propertyKey !== UnknownKey && path5.length < MAX_PATH_DEPTH) {
          if (propertyKey !== this.propertyKey)
            this.expressionsToBeDeoptimized.push(origin);
          return this.object.getLiteralValueAtPath([propertyKey, ...path5], recursionTracker, origin);
        }
        return UnknownValue;
      }
      getLiteralValueAtPathAsChainElement(path5, recursionTracker, origin) {
        if (this.variable) {
          return this.variable.getLiteralValueAtPath(path5, recursionTracker, origin);
        }
        if (this.isUndefined) {
        ...
        return getChainElementLiteralValueAtPath(this, this.object, path5, recursionTracker, origin);
      }
      getReturnExpressionWhenCalledAtPath(path5, interaction, recursionTracker, origin) {
        if (this.variable) {
        ...
        if (propertyKey !== UnknownKey && path5.length < MAX_PATH_DEPTH) {
          if (propertyKey !== this.propertyKey)
            this.expressionsToBeDeoptimized.push(origin);
          return this.object.getReturnExpressionWhenCalledAtPath([propertyKey, ...path5], interaction, recursionTracker, origin);
        }
        return UNKNOWN_RETURN_EXPRESSION;
      }
      hasEffects(context) {
      ...
      hasEffectsOnInteractionAtPath(path5, interaction, context) {
        ...
        if (path5.length < MAX_PATH_DEPTH) {
          return this.object.hasEffectsOnInteractionAtPath([this.getDynamicPropertyKey(), ...path5], interaction, context);
        }
        return true;
      }
      hasEffectsWhenDestructuring(context, destructuredInitPath, init) {
    ...
    FILE_OBJ_PREFIX = "ROLLUP_FILE_URL_OBJ_";
    IMPORT = "import";
    MetaProperty = class extends NodeBase {
      ...
      getReferencedFileName(outputPluginDriver) {
        ...
        if (name === IMPORT) {
          if (metaProperty3?.startsWith(FILE_OBJ_PREFIX)) {
            return outputPluginDriver.getFileName(metaProperty3.slice(FILE_OBJ_PREFIX.length));
          } else if (metaProperty3?.startsWith(FILE_PREFIX)) {
      ...
      includeNode() {
        ...
        if (this.meta.name === IMPORT) {
          ...
          if (metaProperty3?.startsWith(FILE_OBJ_PREFIX)) {
            this.referenceId = metaProperty3.slice(FILE_OBJ_PREFIX.length);
          } else if (metaProperty3?.startsWith(FILE_PREFIX)) {
      ...
      render(code, renderOptions) {
        ...
        const { scope: { context: { module: module2 } }, meta: { name }, metaProperty: metaProperty3, parent, preliminaryChunkId, referenceId, start, end } = this;
        const { id: moduleId, info: { attributes } } = module2;
        if (name !== IMPORT)
          return;
        const chunkId = preliminaryChunkId;
        if (referenceId) {
          const fileName = pluginDriver.getFileName(referenceId);
          const relativePath = normalize((0, import_node_path4.relative)((0, import_node_path4.dirname)(chunkId), fileName));
          const isUrlObject = !!metaProperty3?.startsWith(FILE_OBJ_PREFIX);
        ...
        if (!replacement) {
          replacement = importMetaMechanisms[format]?.(metaProperty3, { chunkId, snippets });
    ...
    getResolveUrl = (path5, asObject, URL2 = "URL") => `new ${URL2}(${path5})${asObject ? "" : ".href"}`;
    ...
    getFileUrlFromFullPath = (path5, asObject) => `require('u' + 'rl').pathToFileURL(${path5})${asObject ? "" : ".href"}`;
    getFileUrlFromRelativePath = (path5, asObject) => getFileUrlFromFullPath(`__dirname + '/${escapeId(path5)}'`, asObject);
    ...
    ExportDefaultVariable = class _ExportDefaultVariable extends LocalVariable {
      constructor(exportDefaultDeclaration3, context) {
        super("default", exportDefaultDeclaration3, exportDefaultDeclaration3.declaration, EMPTY_PATH, context, "other");
        this.hasId = false;
    ...
    NamespaceVariable = class extends Variable {
      ...
      deoptimizeArgumentsOnInteractionAtPath(interaction, path5, recursionTracker) {
        if (path5.length > 1 || path5.length === 1 && interaction.type === INTERACTION_CALLED) {
          ...
          if (typeof key === "string") {
            this.module.getExportedVariablesByName().get(key)?.deoptimizeArgumentsOnInteractionAtPath(interaction, path5.slice(1), recursionTracker);
          } else {
            deoptimizeInteraction(interaction);
          }
        }
      ...
      deoptimizePath(path5) {
        if (path5.length > 1) {
          const key = path5[0];
          if (typeof key === "string") {
            this.module.getExportedVariablesByName().get(key)?.deoptimizePath(path5.slice(1));
      ...
      includePath(path5, context) {
        super.includePath(path5, context);
        this.includeMemberPath(path5, context);
      }
      includeMemberPath(path5, context) {
        if (path5.length > 0) {
          const [name, ...remainingPath] = path5;
          if (typeof name === "string") {
      ...
      setMergedNamespaces(mergedNamespaces) {
        ...
        for (const identifier3 of this.references) {
          const { context } = identifier3.scope;
    ...
    getDynamicNamespaceVariable = (namespace) => Object.create(namespace, {
      includePath: {
        value(path5, context) {
          namespace.includeMemberPath(path5, context);
        }
      }
    });
    SyntheticNamedExportVariable = class _SyntheticNamedExportVariable extends Variable {
      ...
      includePath(path5, context) {
        super.includePath(path5, context);
        this.context.includeVariableInModule(this.syntheticNamespace, [this.name, ...path5], context);
      }
      setRenderNames(baseName, name) {
    ...
    ExternalChunk = class {
      ...
      getFileName() {
        ...
        return this.fileName = (typeof paths === "function" ? paths(this.id) : paths[this.id]) || (this.renormalizeRenderPath ? normalize((0, import_node_path4.relative)(this.inputBase, this.id)) : this.id);
      }
      getImportAttributes(snippets) {
        return this.importAttributes || (this.importAttributes = formatAttributes2(["es", "cjs"].includes(this.options.format) && this.options.externalImportAttributes && this.moduleInfo.attributes, snippets));
      }
    ...
    nodeBuiltins = new Set(builtinModules);
    ...
    getStarExcludes = ({ dependencies, exports: exports$1 }) => {
    ...
    getStarExcludesBlock = (starExcludes, t, { _, cnst, getObject, n: n2 }) => {
      if (starExcludes) {
    ...
    getHoistedExportsBlock = (exports$1, t, snippets) => getExportsBlock(exports$1.filter((expt) => expt.hoisted).map((expt) => ({ name: expt.exported, value: expt.local })), t, snippets);
    getSyntheticExportsBlock = (exports$1, t, snippets) => getExportsBlock(exports$1.filter((expt) => expt.expression).map((expt) => ({ name: expt.exported, value: expt.local })), t, snippets);
    getMissingExportsBlock = (exports$1, t, snippets) => getExportsBlock(exports$1.filter((expt) => expt.local === MISSING_EXPORT_SHIM_VARIABLE).map((expt) => ({ name: expt.exported, value: MISSING_EXPORT_SHIM_VARIABLE })), t, snippets);
    ...
    normalizePathRegExp = new RegExp(`\\${import_path.win32.sep}`, "g");
    ...
    ObjectPattern = class extends NodeBase {
      ...
      declare(kind, destructuredInitPath, init) {
        const variables = [];
        for (const property3 of this.properties) {
          variables.push(...property3.declare(kind, destructuredInitPath, init));
        }
        return variables;
      }
      deoptimizeAssignment(destructuredInitPath, init) {
        for (const property3 of this.properties) {
          property3.deoptimizeAssignment(destructuredInitPath, init);
        }
      }
      deoptimizePath(path5) {
        if (path5.length === 0) {
          for (const property3 of this.properties) {
            property3.deoptimizePath(path5);
          }
        }
      }
      hasEffectsOnInteractionAtPath(_path, interaction, context) {
        for (const property3 of this.properties) {
          if (property3.hasEffectsOnInteractionAtPath(EMPTY_PATH, interaction, context))
            return true;
        }
        return false;
      }
      hasEffectsWhenDestructuring(context, destructuredInitPath, init) {
        for (const property3 of this.properties) {
          if (property3.hasEffectsWhenDestructuring(context, destructuredInitPath, init))
            return true;
        }
        return false;
      }
      includeDestructuredIfNecessary(context, destructuredInitPath, init) {
        ...
        for (const property3 of this.properties.slice(0, -1)) {
          if (lastPropertyIsRestElement && included && !property3.included) {
            property3.includeNode(context);
          }
          included = property3.includeDestructuredIfNecessary(context, destructuredInitPath, init) || included;
        }
        if (!this.included && included) {
          this.includeNode(context);
        }
    ...
    AssignmentExpression = class extends NodeBase {
      ...
      hasEffectsOnInteractionAtPath(path5, interaction, context) {
        return interaction.type === INTERACTION_ASSIGNED && this.left.included || this.right.hasEffectsOnInteractionAtPath(path5, interaction, context);
      }
      include(context, includeChildrenRecursively) {
    ...
    AwaitExpression2 = class extends NodeBase {
      deoptimizePath(path5) {
        this.argument.deoptimizePath(path5);
      }
      hasEffects() {
        if (!this.deoptimized)
          this.applyDeoptimizations();
      ...
      includePath(path5, context) {
        ...
        if (!this.included)
          this.includeNode(context);
        this.argument.includePath(path5, context);
      }
    };
    THEN_PATH = ["then"];
    binaryOperators = {
    ...
    UNASSIGNED$1 = /* @__PURE__ */ Symbol("Unassigned");
    ...
    CallExpressionBase = class extends NodeBase {
      ...
      deoptimizePath(path5) {
        if (path5.length === 0 || this.scope.context.deoptimizationTracker.trackEntityAtPathAndGetIfTracked(path5, this)) {
          return;
        }
    ...
    CallExpression2 = class extends CallExpressionBase {
      ...
      getLiteralValueAtPathAsChainElement(path5, recursionTracker, origin) {
        return getChainElementLiteralValueAtPath(this, this.callee, path5, recursionTracker, origin);
      }
      hasEffects(context) {
        if (!this.deoptimized)
          this.applyDeoptimizations();
    ...
    ChainExpression = class extends NodeBase {
      ...
      getLiteralValueAtPath(path5, recursionTracker, origin) {
        const literalValue = this.expression.getLiteralValueAtPathAsChainElement(path5, recursionTracker, origin);
      ...
      includePath(path5, context) {
        this.included = true;
        this.expression.includePath(path5, context);
      }
      removeAnnotations(code) {
        this.expression.removeAnnotations(code);
      }
    ...
    MultiExpression = class _MultiExpression extends ExpressionEntity {
      ...
      getReturnExpressionWhenCalledAtPath(path5, interaction, recursionTracker, origin) {
        return [
          new _MultiExpression(this.expressions.map((expression) => expression.getReturnExpressionWhenCalledAtPath(path5, interaction, recursionTracker, origin)[0])),
          false
        ];
      }
      hasEffectsOnInteractionAtPath(path5, interaction, context) {
    ...
    ConditionalExpression = class extends NodeBase {
      ...
      getLiteralValueAtPath(path5, recursionTracker, origin) {
        ...
        if (!usedBranch) {
          ...
          const consequentValue = this.consequent.getLiteralValueAtPath(path5, recursionTracker, origin);
          const castedConsequentValue = tryCastLiteralValueToBoolean(consequentValue);
          if (castedConsequentValue === UnknownValue)
            return UnknownValue;
          const alternateValue = this.alternate.getLiteralValueAtPath(path5, recursionTracker, origin);
      ...
      includePath(path5, context) {
        ...
        if (usedBranch === null || this.test.shouldBeIncluded(context)) {
          this.consequent.includePath(path5, context);
          this.alternate.includePath(path5, context);
        } else {
          usedBranch.includePath(path5, context);
        }
      }
      includeCallArguments(interaction, context) {
        const usedBranch = this.getUsedBranch();
    ...
    TrackingScope = class extends BlockScope {
      ...
      addDeclaration(identifier3, context, init, destructuredInitPath, kind) {
        this.hoistedDeclarations.push(identifier3);
        return super.addDeclaration(identifier3, context, init, destructuredInitPath, kind);
      }
    };
    ...
    IfStatement = class _IfStatement extends NodeBase {
      ...
      renderHoistedDeclarations(hoistedDeclarations, code, getPropertyAccess) {
        const hoistedVariables = [
          ...new Set(hoistedDeclarations.map((identifier3) => {
            const variable = identifier3.variable;
    ...
    ObjectPromiseHandler = class {
      ...
      deoptimizeArgumentsOnInteractionAtPath(interaction, path5, recursionTracker) {
        deoptimizeInteraction(interaction);
        if (interaction.type === INTERACTION_CALLED && path5.length === 0 && (isFunctionExpressionNode(interaction.args[1]) || isArrowFunctionExpressionNode(interaction.args[1]))) {
    ...
    ImportExpression = class extends NodeBase {
      ...
      bind() {
        ...
        let callExpression3 = this;
        while (true) {
          if (currentParent.computed || currentParent.object !== callExpression3 || !isIdentifierNode(currentParent.property) || !isCallExpressionNode(currentParent.parent)) {
            break;
          }
          const propertyName = currentParent.property.name;
          callExpression3 = currentParent.parent;
          if (propertyName === "then") {
            const firstArgument = callExpression3.arguments[0];
            ...
            if (isMemberExpressionNode(callExpression3.parent)) {
              currentParent.promiseHandler = new EmptyPromiseHandler();
              currentParent = callExpression3.parent;
              continue;
            }
            if (isExpressionStatementNode(callExpression3.parent)) {
              currentParent.promiseHandler = new EmptyPromiseHandler();
              this.localResolution = { resolution, tracked: true };
              return;
            }
      ...
      deoptimizePath(path5) {
        this.localResolution?.resolution?.namespace.deoptimizePath(path5);
      }
      hasEffects() {
        return true;
      }
      ...
      includePath(path5, context) {
        if (!this.included)
          this.includeNode(context);
        this.localResolution?.resolution?.namespace.includeMemberPath(path5, context);
      }
      initialise() {
        super.initialise();
    ...
    RE_WHITESPACE_TRIM = /^[ \t]*\r?\n[ \t\r\n]*|[ \t]*\r?\n[ \t\r\n]*$/g;
    RE_WHITESPACE_MERGE = /[ \t]*\r?\n[ \t\r\n]*/g;
    ...
    JSXMemberExpression = class extends NodeBase {
      ...
      includePath(path5, context) {
        if (!this.included)
          this.includeNode(context);
        this.object.includePath([this.property.name, ...path5], context);
      }
    };
    ...
    LogicalExpression = class extends NodeBase {
      ...
      getLiteralValueAtPath(path5, recursionTracker, origin) {
        ...
        if (usedBranch) {
          this.expressionsToBeDeoptimized.push(origin);
          return usedBranch.getLiteralValueAtPath(path5, recursionTracker, origin);
        } else if (!this.hasDeoptimizedCache && !path5.length) {
          const rightValue = this.right.getLiteralValueAtPath(path5, recursionTracker, origin);
      ...
      includePath(path5, context) {
        ...
        if (!usedBranch || usedBranch === this.right && this.left.shouldBeIncluded(context)) {
          this.left.includePath(path5, context);
          this.right.includePath(path5, context);
        } else {
          usedBranch.includePath(path5, context);
        }
      }
    ...
    ObjectExpression2 = class extends NodeBase {
      ...
      includePath(path5, context) {
        if (!this.included)
          this.includeNode(context);
        this.getObjectEntity().includePath(path5, context);
      }
      render(code, options, { renderedSurroundingElement } = BLANK) {
    ...
    PanicError2 = class extends NodeBase {
      initialise() {
        ...
        const moduleParseError = logModuleParseError(parseError3, id);
        ...
        for (const node of this.body) {
          if (node.hasEffects(context)) {
            if (this.scope.context.options.experimentalLogSideEffects && !this.hasLoggedEffect) {
              ...
              const { code, log, module: module2 } = this.scope.context;
              log(LOGLEVEL_INFO, logFirstSideEffect(code, module2.id, locate(code, node.start, { offsetLine: 1 })), node.start);
            }
    ...
    Property2 = class extends MethodBase {
      ...
      includeDestructuredIfNecessary(context, destructuredInitPath, init) {
        const path5 = this.getPathInProperty(destructuredInitPath);
        let included = this.value.includeDestructuredIfNecessary(context, path5, init) || this.included;
        if (included || (included = this.key.hasEffects(createHasEffectsContext()))) {
          ...
          if (!this.value.included) {
            ...
            this.value.includeDestructuredIfNecessary(context, path5, init);
          }
        }
        if (!this.included && included) {
          this.includeNode(context);
      ...
      includePath(path5, context) {
        this.included = true;
        this.value.includePath(path5, context);
      }
      markDeclarationReached() {
    ...
    PropertyDefinition = class extends NodeBase {
      ...
      includeNode(context) {
        ...
        for (const decorator3 of this.decorators) {
          decorator3.includePath(UNKNOWN_PATH, context);
        }
      }
    };
    ...
    SequenceExpression = class extends NodeBase {
      ...
      includePath(path5, context) {
        this.included = true;
        this.expressions[this.expressions.length - 1].includePath(path5, context);
      }
      removeAnnotations(code) {
    ...
    SwitchStatement = class extends NodeBase {
      ...
      hasEffects(context) {
        ...
        for (const switchCase3 of this.cases) {
          if (switchCase3.hasEffects(context))
            return true;
      ...
      include(context, includeChildrenRecursively) {
        ...
        for (let caseIndex = this.cases.length - 1; caseIndex >= 0; caseIndex--) {
          ...
          if (isCaseIncluded) {
            switchCase3.include(context, includeChildrenRecursively);
            onlyHasBrokenFlow && (onlyHasBrokenFlow = context.brokenFlow && !context.hasBreak);
            context.hasBreak = false;
            context.brokenFlow = brokenFlow;
          } else {
    ...
    ModuleScope = class extends ChildScope {
      ...
      addDeclaration(identifier3, context, init, destructuredInitPath, kind) {
        if (this.importDescriptions.has(identifier3.name)) {
          context.error(logRedeclarationError(identifier3.name), identifier3.start);
        }
        return super.addDeclaration(identifier3, context, init, destructuredInitPath, kind);
      }
      addExportDefaultDeclaration(exportDefaultDeclaration3, context) {
        const variable = new ExportDefaultVariable(exportDefaultDeclaration3, context);
    ...
    ThisExpression = class extends NodeBase {
      ...
      includePath(path5, context) {
        ...
        if (functionScope && functionScope.functionNode.parent instanceof Property2 && functionScope.functionNode.parent.parent instanceof ObjectExpression2) {
          functionScope.functionNode.parent.parent.includePath(path5, context);
        }
      }
      initialise() {
        super.initialise();
    ...
    UNASSIGNED = /* @__PURE__ */ Symbol("Unassigned");
    ...
    SYMBOL_DISPOSE_PATH = [SymbolDispose];
    SYMBOL_ASYNC_DISPOSE_PATH = [SymbolAsyncDispose];
    ...
    bufferParsers = [
      function panicError2(node, position, buffer) {
        node.message = buffer.convertString(buffer[position]);
      },
      function parseError2(node, position, buffer) {
        node.message = buffer.convertString(buffer[position]);
      },
      function arrayExpression2(node, position, buffer) {
        const { scope } = node;
        node.elements = convertNodeList2(node, scope, buffer[position], buffer);
      },
      function arrayPattern2(node, position, buffer) {
        const { scope } = node;
        node.elements = convertNodeList2(node, scope, buffer[position], buffer);
      },
      function arrowFunctionExpression2(node, position, buffer) {
        const { scope } = node;
        ...
        const parameters = node.params = convertNodeList2(node, scope, buffer[position + 2], buffer);
        scope.addParameterVariables(parameters.map((parameter) => parameter.declare("parameter", EMPTY_PATH, UNKNOWN_EXPRESSION)), parameters[parameters.length - 1] instanceof RestElement2);
        node.body = convertNode2(node, scope.bodyScope, buffer[position + 3], buffer);
      },
      function assignmentExpression2(node, position, buffer) {
        const { scope } = node;
      ...
      function assignmentPattern2(node, position, buffer) {
        const { scope } = node;
        node.left = convertNode2(node, scope, buffer[position], buffer);
        node.right = convertNode2(node, scope, buffer[position + 1], buffer);
      },
      function awaitExpression2(node, position, buffer) {
        const { scope } = node;
        node.argument = convertNode2(node, scope, buffer[position], buffer);
      },
      function binaryExpression2(node, position, buffer) {
        const { scope } = node;
        node.operator = FIXED_STRINGS[buffer[position]];
        node.left = convertNode2(node, scope, buffer[position + 1], buffer);
        node.right = convertNode2(node, scope, buffer[position + 2], buffer);
      },
      function blockStatement2(node, position, buffer) {
        const { scope } = node;
        node.body = convertNodeList2(node, scope, buffer[position], buffer);
      },
      function breakStatement2(node, position, buffer) {
        const { scope } = node;
        const labelPosition = buffer[position];
        node.label = labelPosition === 0 ? null : convertNode2(node, scope, labelPosition, buffer);
      },
      function callExpression2(node, position, buffer) {
        const { scope } = node;
        const flags = buffer[position];
        ...
        node.arguments = convertNodeList2(node, scope, buffer[position + 3], buffer);
      },
      function catchClause2(node, position, buffer) {
        const { scope } = node;
      ...
      function chainExpression2(node, position, buffer) {
        const { scope } = node;
        node.expression = convertNode2(node, scope, buffer[position], buffer);
      },
      function classBody2(node, position, buffer) {
        const { scope } = node;
      ...
      function classDeclaration2(node, position, buffer) {
        const { scope } = node;
        node.decorators = convertNodeList2(node, scope, buffer[position], buffer);
      ...
      function classExpression2(node, position, buffer) {
        const { scope } = node;
        node.decorators = convertNodeList2(node, scope, buffer[position], buffer);
        const idPosition = buffer[position + 1];
      ...
      function conditionalExpression2(node, position, buffer) {
        const { scope } = node;
      ...
      function continueStatement2(node, position, buffer) {
        const { scope } = node;
        const labelPosition = buffer[position];
        node.label = labelPosition === 0 ? null : convertNode2(node, scope, labelPosition, buffer);
      },
      ...
      function decorator2(node, position, buffer) {
        const { scope } = node;
        node.expression = convertNode2(node, scope, buffer[position], buffer);
      },
      function directive2(node, position, buffer) {
        const { scope } = node;
        node.directive = buffer.convertString(buffer[position]);
        node.expression = convertNode2(node, scope, buffer[position + 1], buffer);
      },
      function doWhileStatement2(node, position, buffer) {
        const { scope } = node;
      ...
      function exportAllDeclaration2(node, position, buffer) {
        const { scope } = node;
        ...
        node.attributes = convertNodeList2(node, scope, buffer[position + 2], buffer);
      },
      function exportDefaultDeclaration2(node, position, buffer) {
        const { scope } = node;
        node.declaration = convertNode2(node, scope, buffer[position], buffer);
      },
      function exportNamedDeclaration2(node, position, buffer) {
        const { scope } = node;
        node.specifiers = convertNodeList2(node, scope, buffer[position], buffer);
      ...
      function exportSpecifier2(node, position, buffer) {
        const { scope } = node;
      ...
      function expressionStatement2(node, position, buffer) {
        const { scope } = node;
        node.expression = convertNode2(node, scope, buffer[position], buffer);
      },
      function forInStatement2(node, position, buffer) {
        const { scope } = node;
        node.left = convertNode2(node, scope, buffer[position], buffer);
        node.right = convertNode2(node, scope, buffer[position + 1], buffer);
        node.body = convertNode2(node, scope, buffer[position + 2], buffer);
      },
      function forOfStatement2(node, position, buffer) {
        const { scope } = node;
        const flags = buffer[position];
      ...
      function forStatement2(node, position, buffer) {
        const { scope } = node;
      ...
      function functionDeclaration2(node, position, buffer) {
        const { scope } = node;
        const flags = buffer[position];
        node.async = (flags & 1) === 1;
        ...
        node.id = idPosition === 0 ? null : convertNode2(node, scope.parent, idPosition, buffer);
        const parameters = node.params = convertNodeList2(node, scope, buffer[position + 3], buffer);
        scope.addParameterVariables(parameters.map((parameter) => parameter.declare("parameter", EMPTY_PATH, UNKNOWN_EXPRESSION)), parameters[parameters.length - 1] instanceof RestElement2);
        node.body = convertNode2(node, scope.bodyScope, buffer[position + 4], buffer);
      },
      function functionExpression2(node, position, buffer) {
        const { scope } = node;
        const flags = buffer[position];
        node.async = (flags & 1) === 1;
        node.generator = (flags & 2) === 2;
      ...
      function identifier2(node, position, buffer) {
        node.name = buffer.convertString(buffer[position]);
      },
      function ifStatement2(node, position, buffer) {
        const { scope } = node;
        node.test = convertNode2(node, scope, buffer[position], buffer);
      ...
      function importAttribute2(node, position, buffer) {
        const { scope } = node;
        node.key = convertNode2(node, scope, buffer[position], buffer);
        node.value = convertNode2(node, scope, buffer[position + 1], buffer);
      },
      function importDeclaration2(node, position, buffer) {
        const { scope } = node;
        node.specifiers = convertNodeList2(node, scope, buffer[position], buffer);
        node.source = convertNode2(node, scope, buffer[position + 1], buffer);
        node.attributes = convertNodeList2(node, scope, buffer[position + 2], buffer);
      },
      function importDefaultSpecifier2(node, position, buffer) {
        const { scope } = node;
        node.local = convertNode2(node, scope, buffer[position], buffer);
      },
      function importExpression2(node, position, buffer) {
        const { scope } = node;
      ...
      function importNamespaceSpecifier2(node, position, buffer) {
        const { scope } = node;
        node.local = convertNode2(node, scope, buffer[position], buffer);
      },
      function importSpecifier2(node, position, buffer) {
        const { scope } = node;
      ...
      function jsxAttribute2(node, position, buffer) {
        const { scope } = node;
      ...
      function jsxClosingElement2(node, position, buffer) {
        const { scope } = node;
        node.name = convertNode2(node, scope, buffer[position], buffer);
      },
      function jsxClosingFragment2() {
      },
      function jsxElement2(node, position, buffer) {
        const { scope } = node;
        node.openingElement = convertNode2(node, scope, buffer[position], buffer);
        node.children = convertNodeList2(node, scope, buffer[position + 1], buffer);
      ...
      function jsxExpressionContainer2(node, position, buffer) {
        const { scope } = node;
        node.expression = convertNode2(node, scope, buffer[position], buffer);
      },
      function jsxFragment2(node, position, buffer) {
        const { scope } = node;
      ...
      function literalNumber2(node, position, buffer) {
      ...
      function literalRegExp2(node, position, buffer) {
      ...
      function literalString2(node, position, buffer) {
        node.value = buffer.convertString(buffer[position]);
        const rawPosition = buffer[position + 1];
        node.raw = rawPosition === 0 ? void 0 : buffer.convertString(rawPosition);
      },
      function logicalExpression2(node, position, buffer) {
        const { scope } = node;
        node.operator = FIXED_STRINGS[buffer[position]];
        node.left = convertNode2(node, scope, buffer[position + 1], buffer);
        node.right = convertNode2(node, scope, buffer[position + 2], buffer);
      },
      function memberExpression2(node, position, buffer) {
        const { scope } = node;
        const flags = buffer[position];
      ...
      function metaProperty2(node, position, buffer) {
        const { scope } = node;
        node.meta = convertNode2(node, scope, buffer[position], buffer);
        node.property = convertNode2(node, scope, buffer[position + 1], buffer);
      },
      function methodDefinition2(node, position, buffer) {
        const { scope } = node;
        const flags = buffer[position];
        node.static = (flags & 1) === 1;
        node.computed = (flags & 2) === 2;
        node.decorators = convertNodeList2(node, scope, buffer[position + 1], buffer);
      ...
      function newExpression2(node, position, buffer) {
        const { scope } = node;
        node.annotations = convertAnnotations(buffer[position], buffer);
        node.callee = convertNode2(node, scope, buffer[position + 1], buffer);
        node.arguments = convertNodeList2(node, scope, buffer[position + 2], buffer);
      },
      function objectExpression2(node, position, buffer) {
        const { scope } = node;
        node.properties = convertNodeList2(node, scope, buffer[position], buffer);
      },
      function objectPattern2(node, position, buffer) {
        const { scope } = node;
        node.properties = convertNodeList2(node, scope, buffer[position], buffer);
      },
      function privateIdentifier2(node, position, buffer) {
        node.name = buffer.convertString(buffer[position]);
      },
      function program2(node, position, buffer) {
        const { scope } = node;
        node.body = convertNodeList2(node, scope, buffer[position], buffer);
        node.invalidAnnotations = convertAnnotations(buffer[position + 1], buffer);
      },
      function property2(node, position, buffer) {
        const { scope } = node;
        const flags = buffer[position];
      ...
      function propertyDefinition2(node, position, buffer) {
        const { scope } = node;
        const flags = buffer[position];
        node.static = (flags & 1) === 1;
        node.computed = (flags & 2) === 2;
      ...
      function restElement2(node, position, buffer) {
        const { scope } = node;
        node.argument = convertNode2(node, scope, buffer[position], buffer);
      },
      function returnStatement2(node, position, buffer) {
        const { scope } = node;
        const argumentPosition = buffer[position];
        node.argument = argumentPosition === 0 ? null : convertNode2(node, scope, argumentPosition, buffer);
      },
      function sequenceExpression2(node, position, buffer) {
        const { scope } = node;
        node.expressions = convertNodeList2(node, scope, buffer[position], buffer);
      },
      function spreadElement2(node, position, buffer) {
        const { scope } = node;
        node.argument = convertNode2(node, scope, buffer[position], buffer);
      },
      function staticBlock2(node, position, buffer) {
        const { scope } = node;
        node.body = convertNodeList2(node, scope, buffer[position], buffer);
      },
      function superElement2() {
      },
      function switchCase2(node, position, buffer) {
        const { scope } = node;
        const testPosition = buffer[position];
        node.test = testPosition === 0 ? null : convertNode2(node, scope, testPosition, buffer);
        node.consequent = convertNodeList2(node, scope, buffer[position + 1], buffer);
      },
      function switchStatement2(node, position, buffer) {
        const { scope } = node;
        node.discriminant = convertNode2(node, node.parentScope, buffer[position], buffer);
        node.cases = convertNodeList2(node, scope, buffer[position + 1], buffer);
      },
      function taggedTemplateExpression2(node, position, buffer) {
        const { scope } = node;
        node.tag = convertNode2(node, scope, buffer[position], buffer);
        node.quasi = convertNode2(node, scope, buffer[position + 1], buffer);
      },
      function templateElement2(node, position, buffer) {
        const flags = buffer[position];
        node.tail = (flags & 1) === 1;
      ...
      function templateLiteral2(node, position, buffer) {
        const { scope } = node;
        node.quasis = convertNodeList2(node, scope, buffer[position], buffer);
        node.expressions = convertNodeList2(node, scope, buffer[position + 1], buffer);
      },
      function thisExpression2() {
      },
      function throwStatement2(node, position, buffer) {
        const { scope } = node;
        node.argument = convertNode2(node, scope, buffer[position], buffer);
      },
      function tryStatement2(node, position, buffer) {
        const { scope } = node;
      ...
      function unaryExpression2(node, position, buffer) {
        const { scope } = node;
        node.operator = FIXED_STRINGS[buffer[position]];
        node.argument = convertNode2(node, scope, buffer[position + 1], buffer);
      },
      function updateExpression2(node, position, buffer) {
        const { scope } = node;
        const flags = buffer[position];
      ...
      function variableDeclaration2(node, position, buffer) {
        const { scope } = node;
        node.kind = FIXED_STRINGS[buffer[position]];
        node.declarations = convertNodeList2(node, scope, buffer[position + 1], buffer);
      },
      function variableDeclarator2(node, position, buffer) {
        const { scope } = node;
        node.id = convertNode2(node, scope, buffer[position], buffer);
        const initPosition = buffer[position + 1];
        node.init = initPosition === 0 ? null : convertNode2(node, scope, initPosition, buffer);
      },
      function whileStatement2(node, position, buffer) {
        const { scope } = node;
        node.test = convertNode2(node, scope, buffer[position], buffer);
        node.body = convertNode2(node, scope, buffer[position + 1], buffer);
      },
      function yieldExpression2(node, position, buffer) {
        const { scope } = node;
        const flags = buffer[position];
    ...
    Module = class _Module {
      constructor(graph, id, options, isEntry, moduleSideEffects, syntheticNamedExports, meta, attributes) {
        ...
        this.info = {
          ...
          get hasDefaultExport() {
            ...
            return module2.exportDescriptions.has("default") || reexportDescriptions.has("default");
          },
          id,
          ...
          get importedIdResolutions() {
            return Array.from(sourcesWithAttributes.keys(), (source) => module2.resolvedIds[source]).filter(Boolean);
          },
          get importedIds() {
            return Array.from(sourcesWithAttributes.keys(), (source) => module2.resolvedIds[source]?.id).filter(Boolean);
          },
          get importers() {
            return importers.sort();
          },
      ...
      basename() {
        const base2 = (0, import_node_path4.basename)(this.id);
        const extension = (0, import_node_path4.extname)(this.id);
      ...
      getExportedVariablesByName() {
        ...
        for (const module2 of this.exportAllModules) {
          if (module2 instanceof ExternalModule) {
            exportedVariablesByName.set(`*${module2.id}`, module2.getVariableForExportName("*", {
              importChain: [this.id]
            })[0]);
            continue;
          }
          for (const name of module2.getExportedVariablesByName().keys()) {
      ...
      getImportedJsxFactoryVariable(baseName, nodeStart, importSource) {
        ...
        const module2 = this.graph.modulesById.get(id);
        const [variable] = module2.getVariableForExportName(baseName, { importChain: [this.id] });
        if (!variable) {
      ...
      getVariableFromNamespaceReexports(name, importerForSideEffects, searchedNamesAndModules, importChain) {
        ...
        for (const module2 of this.exportAllModules) {
          ...
          if (module2 instanceof ExternalModule || options?.indirectExternal) {
        ...
        if (foundInternalDeclarations.size > 0) {
          ...
          this.options.onLog(LOGLEVEL_WARN, logNamespaceConflict(name, this.id, foundDeclarationList.map(([, module2]) => module2.id)));
          return [null];
        }
      ...
      includeVariable(variable, path5, context) {
        const { included, module: variableModule } = variable;
        variable.includePath(path5, context);
        if (included) {
        ...
        if (variableModule !== this) {
          ...
          for (const module2 of sideEffectModules) {
            module2.includeModuleInExecution();
          }
        }
      }
      includeVariableInModule(variable, path5, context) {
        this.includeVariable(variable, path5, context);
    ...
    sortExportedVariables = ([a], [b]) => a < b ? -1 : a > b ? 1 : 0;
...
${next}` : out;
    ...
    lowercaseBundleKeys = /* @__PURE__ */ Symbol("bundleKeys");
    FILE_PLACEHOLDER = {
      type: "placeholder"
    };
    ...
    Chunk2 = class _Chunk {
      ...
      canModuleBeFacade(module2, exposedVariables) {
        ...
        for (const exposedVariable of exposedVariables) {
          if (!(exposedVariable.module === module2 || moduleExportNamesByVariable.has(exposedVariable) || exposedVariable instanceof SyntheticNamedExportVariable && moduleExportNamesByVariable.has(exposedVariable.getBaseVariable()))) {
            return false;
          }
        }
        return true;
      ...
      generateFacades() {
        ...
        for (const module2 of entryModules) {
          ...
          for (const facadeName of requiredFacades) {
            facades.push(_Chunk.generateFacade(this.inputOptions, this.outputOptions, this.unsetOptions, this.pluginDriver, this.modulesById, this.chunkByModule, this.externalChunkByModule, this.facadeChunkByModule, this.includedNamespaces, module2, facadeName, this.getPlaceholder, this.bundle, this.inputBase, this.snippets));
          }
        }
        for (const module2 of this.dynamicEntryModules) {
          ...
          if (!this.facadeModule && this.canModuleBeFacade(module2, exposedVariables)) {
            ...
            this.strictFacade = true;
          } else if (!this.facadeChunkByModule.get(module2)?.strictFacade) {
      ...
      getPreliminaryFileName() {
        ...
        if (file) {
          fileName = (0, import_node_path4.basename)(file);
...
`);
          ...
          if (addNonNamespacesAndInteropHelpers || variable.isNamespace) {
            ...
            if (module2 instanceof ExternalModule) {
              ...
              if (addNonNamespacesAndInteropHelpers) {
                ...
                } else if (variable.isNamespace && namespaceInteropHelpersByInteropType[interop(module2.id)] && (this.imports.has(variable) || !this.exportNamesByVariable.get(variable)?.every((name) => name[0] === "*"))) {
        ...
        for (const variable of this.imports) {
          ...
          if (module2 instanceof ExternalModule) {
            ...
            if (imported !== "default" && imported !== "*" && interop(module2.id) === "defaultOnly") {
              return error(logUnexpectedNamedImport(module2.id, imported, false));
            }
          } else {
      ...
      getIncludedDynamicImports() {
        ...
        for (const module2 of this.orderedModules) {
          for (const { node } of module2.dynamicImports) {
            if (!node.included) {
              continue;
            }
            const { resolution } = node;
      ...
      getPreserveModulesChunkNameFromModule(module2) {
        ...
        const extensionName = (0, import_node_path4.extname)(sanitizedId);
        ...
        if (isAbsolute(idWithoutExtension)) {
          if (preserveModulesRoot && (0, import_node_path4.resolve)(idWithoutExtension).startsWith(preserveModulesRoot)) {
          ...
          return this.outputOptions.virtualDirname.replace(/\/$/, "") + "/" + (0, import_node_path4.basename)(idWithoutExtension);
        }
      }
        ...
        for (let exportName of this.getExportNames()) {
          ...
          if (exportName[0] === "*") {
            ...
            if (module2 instanceof Module) {
              ...
              imported = variable.name;
              if (imported !== "default" && imported !== "*" && interop(module2.id) === "defaultOnly") {
                return error(logUnexpectedNamedImport(module2.id, imported, true));
              }
        ...
        for (const module2 of orderedModules) {
          ...
          if (module2.isIncluded() || includedNamespaces.has(module2)) {
            ...
            if (!renderOptions.accessedDocumentCurrentScript && formatsMaybeAccessDocumentCurrentScript.includes(format)) {
              this.accessedGlobalsByScope.get(module2.scope)?.delete(DOCUMENT_CURRENT_SCRIPT);
            }
      ...
      setUpChunkImportsAndExportsForModule(module2) {
        ...
        if (!this.outputOptions.preserveModules && this.includedNamespaces.has(module2)) {
          for (const variable of module2.getExportedVariablesByName().values()) {
            if (variable.included) {
        ...
        for (const { node: { included, resolution } } of module2.dynamicImports) {
    ...
    getHash64 = (input) => (0, import_native2.xxhashBase64Url)(ensureBuffer(input));
    getHash36 = (input) => (0, import_native2.xxhashBase36)(ensureBuffer(input));
    getHash16 = (input) => (0, import_native2.xxhashBase16)(ensureBuffer(input));
    hasherByType = {
      base36: getHash36,
      base64: getHash64,
      hex: getHash16
    ...
    Bundle2 = class {
      ...
      async generateChunks(bundle, getHashPlaceholder) {
        ...
        const executableModule = inlineDynamicImports ? [{ alias: null, modules: includedModules }] : preserveModules ? includedModules.map((module2) => ({ alias: null, modules: [module2] })) : getChunkAssignments(this.graph.entryModules, manualChunkAliasByEntry, experimentalMinChunkSize, this.inputOptions.onLog, typeof manualChunks === "function", onlyExplicitManualChunks);
    ...
    ModuleLoader = class {
      ...
      async addModuleSource(id, importer, module2) {
        ...
        if (cachedModule && !cachedModule.customTransformCache && cachedModule.originalCode === sourceDescription.code && !await this.pluginDriver.hookFirst("shouldTransformCachedModule", [
          ...
          await module2.setSource(cachedModule);
        } else {
          module2.updateOptions(sourceDescription);
          await module2.setSource(await transform(sourceDescription, module2, this.pluginDriver, this.options));
        }
      }
      ...
      async fetchDynamicDependencies(module2, resolveDynamicImportPromises) {
        const dependencies = await Promise.all(resolveDynamicImportPromises.map((resolveDynamicImportPromise) => resolveDynamicImportPromise.then(async ([{ node }, resolvedId]) => {
          ...
          return node.resolution = await this.fetchResolvedDependency(relativeId(resolvedId.id), module2.id, resolvedId);
        })));
        for (const dependency of dependencies) {
          if (dependency) {
      ...
      async fetchModule({ attributes, id, meta, moduleSideEffects, syntheticNamedExports }, importer, isEntry, isPreload) {
        ...
        const module2 = new Module(this.graph, id, this.options, isEntry, moduleSideEffects, syntheticNamedExports, meta, attributes);
      ...
      getResolveDynamicImportPromises(module2) {
        return module2.dynamicImports.map(async (dynamicImport) => {
          const resolvedId = await this.resolveDynamicImport(module2, dynamicImport.argument, module2.id, getAttributesFromImportExpression(dynamicImport.node));
      ...
      getResolveStaticDependencyPromises(module2) {
        return Array.from(module2.sourcesWithAttributes, async ([source, attributes]) => [
          source,
          module2.resolvedIds[source] = module2.resolvedIds[source] || this.handleInvalidResolvedId(await this.resolveId(source, module2.id, EMPTY_OBJECT, false, attributes, module2.info.attributes), source, module2.id, attributes)
        ]);
      }
      ...
      async resolveDynamicImport(module2, specifier, importer, attributes) {
        const resolution = await this.pluginDriver.hookFirst("resolveDynamicImport", [
          specifier,
          importer,
          { attributes, importerAttributes: module2.info.attributes }
        ]);
        ...
        if (resolution == null) {
          ...
          return module2.resolvedIds[specifier] = this.handleInvalidResolvedId(await this.resolveId(specifier, module2.id, EMPTY_OBJECT, false, attributes, module2.info.attributes), specifier, module2.id, attributes);
        }
          ...
          do {
            ...
            for (const module2 of this.modules) {
              if (module2.isExecuted) {
                module2.hasTreeShakingPassStarted = true;
        ...
        for (const module2 of this.modules) {
          for (const importDescription of module2.importDescriptions.values()) {
            if (importDescription.name !== "*") {
              const [variable, options] = importDescription.module.getVariableForExportName(importDescription.name, { importChain: [module2.id] });
              if (!variable) {
                module2.log(LOGLEVEL_WARN, logMissingExport(importDescription.name, module2.id, importDescription.module.id, !!options?.missingButExportExists), importDescription.start);
              }
            }
          }
        }
    ...
    getCache = (config) => config.cache === true ? void 0 : config.cache?.cache || config.cache;
    getIdMatcher = (option) => {
      if (option === true) {
        return () => true;
      }
    ...
    getModuleContext = (config, context) => {
      ...
      if (configModuleContext) {
        ...
        for (const [key, moduleContext] of Object.entries(configModuleContext)) {
          contextByModuleId[(0, import_node_path4.resolve)(key)] = moduleContext;
        }
    ...
    getPreserveModulesRoot = (config) => {
      ...
      return (0, import_node_path4.resolve)(preserveModulesRoot);
    };
    getAmd = (config) => {
      const mergedOption = {
        autoId: false,
    ...
    getExternalImportAttributes = (config, inputOptions) => {
    ...
    getManualChunks = (config, inlineDynamicImports, preserveModules) => {
    ...
    getSourcemapFileNames = (config, unsetOptions) => {
    ...
    Symbol.asyncDispose ?? (Symbol.asyncDispose = /* @__PURE__ */ Symbol("Symbol.asyncDispose"));
    ...
    ({ bold, cyan, dim, red } = pc.createColors(import_node_process.env.FORCE_COLOR !== "0" && !import_node_process.env.NO_COLOR));
  }
});
...
function normalizeFsPath(p) {
  return toPosix(import_node_path.default.resolve(p));
}
...
function fileNameFromRoute(routePath, cleanUrls) {
  ...
  const base2 = normalized.slice(1);
  return cleanUrls ? `${base2}/index.html` : `${base2}.html`;
}
...
async function discoverEntryPages(root, options) {
  ...
  return files.sort().map((absolutePath) => {
    ...
    const relativePath = toPosix(import_node_path2.default.relative(root, entryPath));
    const relativeFromPagesDir = toPosix(import_node_path2.default.relative(import_node_path2.default.join(root, pagesDir), entryPath));
...
async function buildRenderBundle(args) {
  ...
  const hash = (0, import_node_crypto.createHash)("sha256").update(source).digest("hex").slice(0, 12);
  const bundlePath = import_node_path6.default.join(cacheDir, `render-${hash}.mjs`);
  ...
  await import_promises2.default.writeFile(bundlePath, chunk.code, "utf8");
  await bundle.close();
  return bundlePath;
}
...
async function importManifest(bundlePath) {
  const mod = await import((0, import_node_url.pathToFileURL)(bundlePath).href + `?t=${Date.now()}`);
  return mod.manifest;
}
function htPages(options = {}) {
  ...
  return {
    ...
    configureServer(_server) {
      ...
      loadDevPages().catch((error2) => {
        server?.config.logger.error(String(error2));
      });
      return () => {
        server = null;
      };
    ...
    async generateBundle() {
      const entries = await discoverEntryPages(root, options);
      const cacheDir = import_node_path7.default.join(root, "node_modules/.cache/vite-plugin-htjs-pages");
      ...
      const limit = (0, import_p_limit.default)(options.renderConcurrency ?? 8);
